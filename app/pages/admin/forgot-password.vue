  ...
  <div class="login-form-panel min-vh-100 d-flex align-items-center justify-content-center p-3 p-md-5 position-relative overflow-hidden">
    ...
    <div class="login-card p-4 p-md-5 login-fade-in">
      <div class="text-center mb-4">
        ...
        <h1 class="h4 font-display mt-1 mb-1">Forgot your password?</h1>
        <p class="text-muted small mb-0">Enter your admin email and we'll send you a reset link.</p>
      </div>

      <form v-if="!sent" @submit.prevent="submit">
        <div class="login-field mb-4" :class="{ 'is-focused': focused }">
      ...
      <div v-else class="text-center login-fade-in">
        ...
        <p class="mb-0">If an account exists for <strong>{{ email }}</strong>, a reset link is on its way. Check your inbox.</p>
      </div>

      <p class="text-center small mt-4 mb-0">
        <NuxtLink to="/admin/login" class="login-link"><i class="bi bi-arrow-left me-1" />Back to login</NuxtLink>
      </p>
    </div>
  </div>
</template>
...
async function submit() {
  ...
  error.value = ''
  try {
    await useApi('/api/auth/forgot-password', { method: 'POST', body: { email: email.value } })
    sent.value = true
  } catch (e) {
