<template>
  <div>
    <div class="admin-page-header">
      <div>
        <h1 class="h4 font-display mb-0">{{ customer.fullName }} <span v-if="customer.isBlocked" class="badge status-badge status-badge--inactive ms-1">Blocked</span></h1>
        <p class="admin-page-header__subtitle">{{ customer.phone }}<span v-if="customer.email"> &middot; {{ customer.email }}</span></p>
      </div>
      <div class="admin-page-header__actions">
        <button class="btn btn-amber" @click="showModal = true"><i class="bi bi-pencil me-1" />Edit Customer</button>
      </div>
    </div>
    ...
    <div class="card">
      <div v-if="!customer.bookings.length" class="admin-empty-state">
    ...
    <AdminModal v-model="showModal" title="Edit Customer">
      <form id="customer-form" @submit.prevent="save">
        <div class="mb-2">
      ...
      <template #footer>
        ...
        <button type="submit" form="customer-form" class="btn btn-amber">Save Changes</button>
      </template>
    </AdminModal>
  </div>
</template>
