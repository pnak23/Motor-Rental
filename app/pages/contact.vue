<template>
  <div>
    <PageHeader eyebrow="Get in Touch" :title="`Contact ${settings?.businessName || 'Us'}`" />

    <div class="container py-5">
    <div class="row g-5">
      <div class="col-lg-5" v-reveal>
        <div class="card p-4 mb-3">
          <h3 class="h6 font-display mb-3">Reach us directly</h3>
          <ul class="list-unstyled d-flex flex-column gap-3 mb-0">
            ...
            <li v-if="telegramLink" class="d-flex align-items-center gap-3">
              ...
              <a :href="telegramLink" target="_blank" rel="noopener" class="text-decoration-none">Chat on Telegram</a>
            </li>
            <li v-if="whatsappLink" class="d-flex align-items-center gap-3">
              <span class="contact-icon"><i class="bi bi-whatsapp" /></span>
              <a :href="whatsappLink" target="_blank" rel="noopener" class="text-decoration-none">Chat on WhatsApp</a>
            </li>
          </ul>
        </div>
      </div>
      ...
      <div class="col-lg-7" v-reveal>
        <div class="card p-4">
          <h3 class="h6 font-display mb-3">Send us a message</h3>
          <form @submit.prevent="submit">
            <div class="row g-2">
              <div class="col-md-6"><input v-model="form.name" required class="form-control mb-2" placeholder="Name *" /></div>
              <div class="col-md-6"><input v-model="form.email" required type="email" class="form-control mb-2" placeholder="Email *" /></div>
              <div class="col-12"><input v-model="form.phone" class="form-control mb-2" placeholder="Phone (optional)" /></div>
              <div class="col-12"><textarea v-model="form.message" required rows="4" class="form-control mb-2" placeholder="Message *" /></div>
            </div>
            <button type="submit" class="btn btn-amber" :disabled="submitting">
              <span v-if="submitting" class="spinner-border spinner-border-sm me-2" />Send Message
            </button>
            <p v-if="sent" class="text-success small mt-2 mb-0"><i class="bi bi-check-circle me-1" />Message sent — we'll get back to you soon!</p>
...
async function submit() {
  ...
  try {
    ...
    error.value = e instanceof Error ? e.message : 'Could not send your message. Please try again.'
  } finally {
