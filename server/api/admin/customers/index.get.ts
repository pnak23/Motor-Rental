...
export default defineEventHandler(async (event) => {
  ...
  const rows = await query(
    `SELECT c.*,
       (SELECT COUNT(*) FROM bookings b WHERE b."customerId" = c.id)::int as "totalBookings",
       (SELECT COALESCE(SUM(total),0) FROM bookings b WHERE b."customerId" = c.id AND b.status <> 'CANCELLED' AND b.status <> 'REJECTED') as "totalSpent"
     FROM customers c
     ${whereSql}
