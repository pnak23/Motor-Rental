  ...
  const [
    ...
    queryOne<{ subtotal: string; discount: string; deliveryFee: string; additionalCharges: string; total: string; paidAmount: string; count: string }>(
      `SELECT
        ...
        COUNT(*)::text as count
       FROM bookings
       WHERE status NOT IN ('CANCELLED','REJECTED') AND "createdAt"::date BETWEEN $1 AND $2`,
      [from, to]
    ),
    ...
    query<{ paymentStatus: string; count: string; total: string }>(
      `SELECT "paymentStatus", COUNT(*)::text as count, COALESCE(SUM(total),0)::text as total
       FROM bookings WHERE "createdAt"::date BETWEEN $1 AND $2
       GROUP BY 1 ORDER BY 1`,
      [from, to]
    ),
    query<{ status: string; count: string }>(
      `SELECT status, COUNT(*)::text as count FROM bookings WHERE "createdAt"::date BETWEEN $1 AND $2 GROUP BY 1`,
      [from, to]
    ),
    queryOne<{ avgDays: string }>(
      `SELECT COALESCE(AVG(EXTRACT(DAY FROM ("returnDate" - "pickupDate"))),0)::text as "avgDays"
       FROM bookings WHERE status NOT IN ('CANCELLED','REJECTED') AND "createdAt"::date BETWEEN $1 AND $2`,
      [from, to]
    ),
    ...
    queryOne<{ newCustomers: string; returningCustomers: string }>(
      `SELECT
        (SELECT COUNT(*) FROM customers WHERE "createdAt"::date BETWEEN $1 AND $2)::text as "newCustomers",
        (SELECT COUNT(DISTINCT c.id) FROM customers c JOIN bookings b ON b."customerId" = c.id
          WHERE b."createdAt"::date BETWEEN $1 AND $2 AND c."createdAt"::date < $1)::text as "returningCustomers"`,
      [from, to]
    ),
